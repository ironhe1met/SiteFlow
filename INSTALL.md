...
## 5. Завантаження проєкту з GitHub

```bash
cd /var/www
git clone https://github.com/ironhe1met/SiteFlow.git
cd SiteFlow
```
...
```bash
cd /var/www/SiteFlow
pm2 start server.js --name siteflow
pm2 save
pm2 startup
```
...
```bash
pm2 restart siteflow   # перезапустити після змін
pm2 stop siteflow      # зупинити
pm2 delete siteflow    # видалити з PM2
pm2 logs siteflow      # логи
```

## Оновлення проєкту з GitHub

```bash
cd /var/www/SiteFlow
git pull origin main
npm install
pm2 restart siteflow
```
